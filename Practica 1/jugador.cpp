...
bool ComportamientoJugador::NoPuedesPasar(int fila, int col, Sensores sensor)
{
	...
	cout<<sensor.superficie[2]<<endl;
	
	if(bien_situado){
		if (sensor.superficie[2] == 'l' || sensor.superficie[2] == 'a' || mapaResultado[fila][col] == 'B' && sensor.objetoActivo!='2' || mapaResultado[fila][col] == 'M' || mapaResultado[fila][col] == 'A' && sensor.objetoActivo!='1' || mapaResultado[fila][col]=='P' || mapaResultado[fila][col]=='D' ){
			no_puedes_pasar = true; 
		}

	}
	else{ // no está bien situado 
		if (sensor.superficie[2] == 'l' ||sensor.superficie[2] == 'a' || mapaPrueba[fila][col] == 'B' && sensor.objetoActivo!='2' || mapaPrueba[fila][col] == 'M' || mapaPrueba[fila][col] == 'A' && sensor.objetoActivo!='1'|| mapaPrueba[fila][col]=='P' || mapaPrueba[fila][col]=='D' ){
			no_puedes_pasar = true; 
		}
